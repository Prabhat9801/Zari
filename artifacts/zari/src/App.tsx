...
import { useAuthActions, useCurrentUser, useDesign, useDesigners, useDesignerProfile, useDesigns, useEditDesign, useGenerateDesign, useGuestBootstrap, useOrder, useOrders } from '@/hooks/useZari';
import { designsService } from '@/services/designs';
import { formatINR, formatRange, imageForView } from '@/types';
import { usePlatformConfig, useScoringPolicy } from '@/hooks/usePlatform';
import VersionTree from '@/components/versions/VersionTree';
...
/** Carries its own stylesheet, and only the studio opens it. */
const BudgetOptimizer = lazy(() => import('@/components/budget/BudgetOptimizer'));
 ...
 *
 * It used to bail out when VITE_API_URL was unset, which is exactly the build
 * where EVERY screen is sample content — so the chip appeared only in the one
 * narrow case where the URL was set but the server was down, and never in the
 * demo deployment. The whole point is to be honest in that build.
 */
function DemoNote({ isLive }: { isLive: boolean }) {
  if (isLive) return null;
  const reason = isApiConfigured
    ? 'The server is unreachable, so Zari is showing sample content.'
    : 'Zari is running on sample content until the API is connected.';
  return <span className="demo-note" title={reason} data-testid="status-demo-data">Sample data</span>;
}
...
function Concepts() {
  ...
  // No invented fallback price. An unpriced concept says so.
  const estimate = chosen?.estimateMin != null && chosen?.estimateMax != null
    ? formatRange(chosen.estimateMin, chosen.estimateMax)
    : null;
  return <AppShell><div className="app-heading"><div><div className="eyebrow">Four directions · ready <DemoNote isLive={isLive} /></div><h1>Find the one that feels like you.</h1><p>Each direction can be refined, priced, and made by a real designer.</p></div><Button onClick={open} testId="button-open-selected-concept">Open selected <ArrowRight size={15} /></Button></div><section className="surface" style={{ padding: 13 }}><div className="concepts" style={{ marginTop: 0 }}>{concepts.map((concept, index) => <button className={`concept-card ${selected === index ? 'selected' : ''}`} onClick={() => setSelected(index)} key={concept.id} data-testid={`button-concept-${index}`}><div className="concept-visual"><GarmentArt tone={concept.tone} url={concept.coverUrl} /></div><div className="concept-copy"><strong>{concept.name}</strong><span>{index === selected ? 'SELECTED' : `0${index + 1}`}</span></div></button>)}</div></section><div className="surface" style={{ marginTop: 20, padding: 17, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 15, flexWrap: 'wrap' }}><span className="muted" style={{ fontSize: 12 }}>Selected direction: <b className="ink">{chosen?.name}</b>{estimate ? <> · Estimate {estimate}</> : ' · Not priced yet'}</span><Button onClick={open} variant="coral" testId="button-continue-studio">Continue to studio <ArrowRight size={15} /></Button></div></AppShell>;
}
...
const MILESTONE_DONE = new Set(['DONE']);

function OrderDetail() {
  const params = useParams<{ orderId: string }>();
  const { data: orders, isLive: listLive } = useOrders();
  // The full order: real amounts, real milestones, real escrow state. The page
  // used to render a fixed ₹6,400 and a fixed timeline on top of a live title,
  // so a real order showed someone else's price.
  const { data: full, isLive } = useOrder(params.orderId);
  const summary = orders.find((o) => o.id === params.orderId || o.code === params.orderId) ?? orders[0];
  const [toast, setToast] = useState<ToastState>(null);

  const studio = full?.designer?.studioName ?? summary?.designerName ?? 'your designer';
  const advancePercent = full?.advancePercent ?? 40;
  const dateOf = (iso?: string | null) => (iso ? new Date(iso).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) : null);

  return <AppShell><div className="app-heading"><div><div className="eyebrow"><Link href="/orders" data-testid="link-back-orders">Orders</Link> / {full?.code ?? summary?.code ?? params.orderId} <DemoNote isLive={isLive || listLive} /></div><h1>Order {full?.code ?? summary?.code ?? params.orderId}</h1><p>{studio}{dateOf(full?.promisedDate) ? ` · Due ${dateOf(full?.promisedDate)}` : ''}</p></div><span className="status-pill">{summary?.statusLabel ?? full?.status?.replace(/_/g, ' ') ?? 'IN PRODUCTION'}</span></div><div className="order-detail"><section className="surface order-summary"><div className="order-thumb" style={{ width: 120, height: 135 }}><GarmentArt tone={summary?.tone} /></div><h2>{full?.version?.design?.title ?? summary?.title}</h2>{full ? <><div className="cost-row"><span>Final quote</span><strong>{formatINR(full.finalPrice)}</strong></div><div className="cost-row"><span>First payment · {advancePercent}%</span><span>{formatINR(full.advanceAmount)}{full.escrow?.advancePaid ? ' paid' : ' due'}</span></div><div className="cost-row"><span>Balance after QC · {100 - advancePercent}%</span><span>{formatINR(full.balanceAmount)}</span></div></> : <p className="muted" style={{ fontSize: 12 }}>Sign in to see the amounts on this order.</p>}<div className="timeline">{(full?.milestones ?? []).map((milestone) => <div className={`timeline-row ${MILESTONE_DONE.has(milestone.status) ? 'done' : ''}`} key={milestone.id}><span className="timeline-dot"></span><div><strong>{milestone.title}</strong><small>{[dateOf(milestone.occurredAt), milestone.note].filter(Boolean).join(' · ') || (milestone.status === 'IN_PROGRESS' ? 'In progress' : 'Not started yet')}</small></div></div>)}</div><Button variant="ghost" onClick={() => setToast({ message: `Message sent to ${studio}.` })} testId="button-message-order"><MessageCircle size={14} /> Message {studio}</Button></section><aside style={{ display: 'grid', alignContent: 'start', gap: 15 }}><div className="escrow surface"><div className="eyebrow" style={{ color: 'hsl(39 42% 86%)' }}>Protected payment</div><h3>Escrow, not blind trust.</h3><p>{full?.escrow?.explanation ?? 'Your payment stays protected while the garment is being made. Your designer receives the balance only after Zari quality control passes.'}</p>{full ? <><div className="escrow-row"><span>At acceptance</span><strong>{advancePercent}% · {formatINR(full.advanceAmount)}</strong></div><div className="escrow-row"><span>After Zari QC</span><strong>{100 - advancePercent}% · {formatINR(full.balanceAmount)}</strong></div></> : null}</div><div className="surface studio-card"><div className="eyebrow">When it arrives</div><h3 style={{ font: '400 26px var(--app-font-serif)', margin: '10px 0 6px' }}>Your fit window</h3><p className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>{full?.fitGuarantee?.explanation ?? `You have ${full?.fitGuarantee?.windowDays ?? 7} days to try it on. If something needs adjusting, your first alteration is free.`}</p><Button variant="soft" onClick={() => setToast({ message: 'We will remind you when your fit window opens.' })} testId="button-fit-reminder">Remind me</Button></div></aside></div><Toast toast={toast} setToast={setToast} /></AppShell>;
}
